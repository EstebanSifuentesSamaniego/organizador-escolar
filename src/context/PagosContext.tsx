...
interface PagosContextType {
  ...
  eliminarPago: (id: string) => Promise<Pago | null>;
...
export function PagosProvider({ children }: { children: ReactNode }) {
  ...
  // --- Eliminar pago (devuelve copia para poder deshacer) ---
  const eliminarPago = async (id: string): Promise<Pago | null> => {
    ...
    const copia: Pago = {
      ...
      monto: data.monto,
      padresIds: normalizePadresIds(data.padresIds),
  ...
  const marcarPagado = async (pagoId: string, padreId: string) => {
    ...
    const nuevosIds = Array.from(
      new Set([...pagoActual.padresIds, String(padreId)])
    );

    await updatePago(pagoId, { padresIds: nuevosIds });
  };
  ...
  const desmarcarPago = async (pagoId: string, padreId: string) => {
    ...
    const nuevosIds = pagoActual.padresIds.filter((id) => id !== String(padreId));
